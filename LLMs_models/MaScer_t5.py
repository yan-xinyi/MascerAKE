...
def get_features(item, word, mode=str):
    """
    获取词的认知特征，处理OOV情况
...
def load_data(args):
    ...
    for key1 in train_file.keys():
        ...
        for item in items:
            ...
            word = item[0]
            sens += ' '
            fs = get_features(item, word, "train")
            features.append(fs)
            tags.append(item[-1])
            nums += 1
    ...
    for key2 in test_file.keys():
        ...
        for item in items:
            ...
            word = item[0]
            sens += ' '
            fs = get_features(item, word, "test")
            features.append(fs)
            tags.append(item[-1])
            nums += 1
...
class T5NerModel(nn.Module):
    def __init__(self, num_labels, tokenizer, vocab_feature_dict, oov_feature_dict, weight='t5-base', dp=0.1, fasttext_dict={}, fs_combine="base"):
        ...
        self.weight = weight
        if self.weight == "t5-base":
            self.t5_hidden_size = 768
        if self.weight == "t5-large":
            self.t5_hidden_size = 1024
        self.feature_dim = {
            "et": 17,
            "eeg": 8,
            "et+eeg": 25,
        ...
        # 跨模态融合模块（Cross-Attention）
        if self.feature_dim > 0:
            self.cross_attention = nn.MultiheadAttention(embed_dim=self.t5_hidden_size,
        ...
        self.classifier = nn.Linear(self.t5_hidden_size + self.feature_dim, num_labels)
        ...
        for syn in synonyms:
            for lemma in syn.lemmas():
                ...
                if synonym in self.vocab_feature_dict:
                    return self.vocab_feature_dict[synonym]
        return None

    def replace_oov_embeddings_and_features(self, input_ids, extra_features):
        with torch.no_grad():
            ...
            for i in range(batch_size):
                ...
                for tok in tokens:
                    if tok.startswith("▁"):
                        ...
                        current_word += tok
                    word_map.append(current_word.lower())

                for j, word in enumerate(word_map):
                    if word in self.oov_feature_dict:
                        syn_feat = self.get_synonym_embedding(word)
                        if synonym_feature is not None and fasttext_feature is not None:  # 两种策略的结合
                            combined_feat = [
    ...
    def forward(self, input_ids, attention_mask, extra_features, labels=None):
        ...
        encoder_outputs = outputs.last_hidden_state  # [B, L, 1024]

        # Step 2: 准备认知特征
        if isinstance(extra_features, list):
            extra_features = torch.stack(extra_features, dim=1)
        extra_features = extra_features.to(encoder_outputs.device)

        # Step 3: OOV 替换
        if self.feature_dim > 0:
            self.replace_oov_embeddings_and_features(input_ids, extra_features)

        # Step 4: 融合语言与认知模态
        if self.fs_combine == "base":
            fused_outputs = encoder_outputs
        if self.fs_combine == "et":
            fused_outputs = torch.cat([encoder_outputs, extra_features[:, :, :17]], dim=-1)
        if self.fs_combine == "eeg":
            fused_outputs = torch.cat([encoder_outputs, extra_features[:, :, 17:25]], dim=-1)
        if self.fs_combine == "et+eeg":
            fused_outputs = torch.cat([encoder_outputs, extra_features[:, :, :25]], dim=-1)  # # 映射回 [B, L, 1024]

        fused_outputs = self.dropout(fused_outputs)

        # Step 5: 线性分类器输出 token-level 标签
        logits = self.classifier(fused_outputs)  # [B, L, num_labels]
...
def main():
    ...
    # 数据集参数
    parser.add_argument('--dn', type=str, default='ELE', choices=['ELE', 'GT'],
                        help='数据集名称: ELE, GT')

    # 特征组合参数
    parser.add_argument('--fs_combine', type=str, default='et', choices=['eeg', 'et', 'et+eeg', 'base'],
                        help='特征组合方式: eeg (脑电), et (眼动), et+eeg (全部), base (不加特征)')
    # 模型参数
    parser.add_argument('--weight', type=str, default='t5-large', help='BERT模型名称')
    ...
    logger.info(f"BERT模型: {args.weight}")
    ...
    # Tokenizer
    tokenizer = T5TokenizerFast.from_pretrained(args.weight)
    ...
    # 初始化模型
    logger.info(f"初始化BERT模型 ({args.weight})...")
    torch.manual_seed(1)

    model = T5NerModel(num_labels = 6, tokenizer = tokenizer, vocab_feature_dict = vocab_feature_dict, oov_feature_dict = oov_feature_dict,
                 weight = args.weight, dp = args.dropout, fasttext_dict=fasttext_dict, fs_combine=args.fs_combine).to(device)
    # 如果仅进行测试
    if args.test_only:
