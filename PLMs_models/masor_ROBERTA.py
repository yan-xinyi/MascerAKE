...
class BertNerModel(nn.Module):
    ...
    def __init__(self, weight, num_labels=6, fs_num=0, tokenizer=None, vocab_feature_dict={}, oov_feature_dict={},
                 fasttext_dict={}, fs_combine="base"):
        super(BertNerModel, self).__init__()

        self.albert = RobertaModel.from_pretrained(weight)
        ...
        self.classifier = nn.Linear(768 + fs_num, num_labels)
    ...
    def replace_oov_embeddings_and_features(self, input_ids, pooled_outputs, extra_features,  wordnet_weight=0.6, fasttext_weight=0.4):
        """
        替换 OOV 词的特征表示，融合 WordNet 同义词嵌入和 FastText 向量
        """
        synonym_feature_count = 0
        with torch.no_grad():
                    ...
                    if word in self.oov_feature_dict:           # 是 OOV 词
                        ...
                        if synonym_feature is not None and fasttext_feature is not None:
                            ...
                            extra_features[i, j, :] = torch.tensor(fasttext_feature, dtype=extra_features.dtype,
                                                                   device=extra_features.device)
        # 统计信息输出或返回（根据需要调整）
        ratio = synonym_feature_count / 33583  # 82736
        print(f"synonym_feature 不为空的单词数：{synonym_feature_count}")
        print(f"在oov_feature_dict中的占比：{ratio:.2%}")
...
def main():
    ...
    # 特征组合参数
    parser.add_argument('--fs_combine', type=str, default='et+eeg', choices=['eeg', 'et', 'et+eeg', 'base', "beta", "gamma", "beta+gamma"],
    ...
    # 模型参数
    parser.add_argument('--bert_weight', type=str, default='roberta-base',
                        help='BERT模型名称')
    ...
    parser.add_argument('--model_path', type=str, default='./best_roberta_model1.pt', help='保存模型路径')
    ...
    # 初始化分词器
    tokenizer = RobertaTokenizerFast.from_pretrained(args.bert_weight)
    ...
    elif args.fs_combine == 'et+eeg':
        fs_num = 25  # 使用所有特征
        logger.info("使用全部特征 (25维)")
    elif args.fs_combine == 'base':
        fs_num = 0  # 不使用特征
        logger.info("不使用认知特征")
    elif args.fs_combine == 'beta':
        fs_num = 2  # 使用所有特征
        logger.info("使用beta特征 (2维)")
    elif args.fs_combine == 'gamma':
        fs_num = 2  # 使用所有特征
        logger.info("使用gamma特征 (2维)")
    elif args.fs_combine == 'beta+gamma':
        fs_num = 4  # 使用所有特征
        logger.info("使用bg特征 (4维)")

    # 初始化模型
    logger.info(f"初始化BERT模型 ({args.bert_weight})...")
    torch.manual_seed(1)
    model = BertNerModel(
        weight=args.bert_weight,
        num_labels=6,
        fs_num=fs_num,
        tokenizer=tokenizer,
